...
AS 
SELECT book_id, title, authors, price 
FROM books_silver
...
LATERAL EXPLODE(o.books) AS b(book) INNER JOIN 
current_books AS c ON b.book.book_id = c.book_id;
...
FROM books_sales
GROUP BY authors, window(order_timestamp, '5 minutes', '5 minutes', '2 minutes')
